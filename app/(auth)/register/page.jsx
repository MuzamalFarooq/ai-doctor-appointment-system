...
export default function RegisterPage() {
  ...
  return (
    <>
      ...
      <p className="text-gray-400 text-sm text-center mb-6">Join 10,000+ patients and 500+ doctors</p>

      {/* Role Selector */}
      <div className="grid grid-cols-2 gap-3 mb-5">
        {[{ value: 'PATIENT', label: 'Patient', icon: UserCheck, desc: 'Book appointments' }, { value: 'DOCTOR', label: 'Doctor', icon: Stethoscope, desc: 'Join as provider' }].map(({ value, label, icon: Icon, desc }) => (
          <button key={value} type="button" onClick={() => setRole(value)}
            className={`p-3 rounded-xl border-2 transition-all text-left ${role === value ? 'border-primary-400 bg-primary-500/20' : 'border-white/20 bg-white/5 hover:bg-white/10'}`}>
            <Icon className={`w-5 h-5 mb-1 ${role === value ? 'text-primary-400' : 'text-gray-400'}`} />
      ...
      {role === 'DOCTOR' && (
        <div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 mb-4">
          ...
          <p className="text-xs text-amber-300">Doctor accounts require admin approval before you can start accepting patients. You&apos;ll be notified by email.</p>
        </div>
      )}

      <button onClick={handleGoogle} className="w-full flex items-center justify-center gap-3 py-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-white font-medium transition-all mb-4 text-sm">
        ...
        <div>
          <div className="relative">
            ...
            <input type={showPassword ? 'text' : 'password'} placeholder="Password" {...register('password')}
              className="w-full pl-10 pr-10 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-400 text-sm" />
          ...
          {password && (
            <div className="flex gap-2 mt-2">
              {strengthChecks.map(({ label, ok }) => (
                <div key={label} className={`flex items-center gap-1 text-xs ${ok ? 'text-green-400' : 'text-gray-500'}`}>
